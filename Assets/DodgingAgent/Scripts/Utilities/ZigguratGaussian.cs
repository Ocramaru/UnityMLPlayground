// Adapted  from Redzen library: https://github.com/colgreen/Redzen to work with UnityEngine.Random
...
namespace DodgyBall.Scripts.Utilities
{
    ...
    public static class ZigguratGaussian
    {
        ...
        public static void Sample(out double x)
        {
            while(true)
            {
                ...
                x = u2 * __INCR * __x[s];
                if(__y[s-1] + ((__y[s] - __y[s-1]) * NextDouble()) < GaussianPdfDenorm(x))
                {
                    SetSignBit(ref x, ref signBit);
                    return;
                }
        ...
        /// <param name="x">Reference to a variable to store the new sample value in.</param>
        public static void Sample(double mean, double stdDev, out double x)
        {
            Sample(out x);
        ...
        public static void Sample(Span<double> span)
        {
        ...
        /// <param name="stdDev">Distribution standard deviation.</param>
        /// <param name="span">The span to fill with random samples.</param>
        public static void Sample(double mean, double stdDev, Span<double> span)
        {
            for(int i=0; i < span.Length; i++)
            {
                Sample(out span[i]);
        ...
        /// <summary>
        /// Helper to generate a random ulong using UnityEngine.Random
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static ulong NextULong()
        {
            // Generate 64 random bits using two 32-bit random values
            uint high = (uint)(Random.value * uint.MaxValue);
            uint low = (uint)(Random.value * uint.MaxValue);
            return ((ulong)high << 32) | low;
        }

        /// <summary>
        /// Helper to get a random double [0,1) using UnityEngine.Random
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        /// <summary>
        /// Helper to get a random double (0,1] (never zero) using UnityEngine.Random
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        /// </summary>
        private static void SampleTail(out double x)
        {
            double y;
            do
            {
                ...
                x = -Math.Log(NextDoubleNonZero()) / __R;
